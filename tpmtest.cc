...
struct MainLoop {
  ...
  inline void run() { g_main_loop_run(loop); }
};
...
static void genkey(const char *filename = "private.key",
                   const char *password = "password", size_t mbits = 2048) {

  auto pkey = make_handle(
      EVP_PKEY_Q_keygen(NULL, "provider=tpm2", "RSA", mbits), EVP_PKEY_free);
  if (!pkey) {
  ...
  if (PEM_write_PrivateKey(
          file.get(), pkey.get(), EVP_get_cipherbyname("aes-256-cbc"),
          (unsigned char *)password, (password ? strlen(password) : 0), NULL,
          NULL) != 1) {
    throw runtime_error("Error writing private key");
  }

  auto bio = make_handle(
      BIO_new_file(get_filename(filename, "pub").c_str(), "w"), BIO_free_all);
  if (!bio) {
...
static void gencsr(const char *filename = "private.key",
  ...
  {
    auto bio = make_handle(
        BIO_new_file(get_filename(filename, "pub").c_str(), "r"), BIO_free_all);
    if (!bio) {
      throw runtime_error("Error opening public key file");
    }

    auto pkey = make_handle(PEM_read_bio_PUBKEY(bio.get(), NULL, NULL, NULL),
                            EVP_PKEY_free);
    if (!pkey) {
  ...
  for (const auto &nid : nids) {
    X509_NAME_add_entry_by_NID(name, nid.type, MBSTRING_ASC,
                               (unsigned char *)nid.value, -1, -1, 0);
  }
  ...
  // Sign the request
  {
    ...
    auto pkey = make_handle(
        PEM_read_bio_PrivateKey(bio.get(), NULL, NULL, (void *)password),
        EVP_PKEY_free);
    if (!pkey) {
  ...
  auto csrfile =
      make_handle(fopen(get_filename(filename, "csr").c_str(), "wb"), fclose);
  if (!csrfile) {
...
static void added_cb(GObject *client, GAsyncResult *result,
  ...
  NMRemoteConnection *remote =
      nm_client_add_connection_finish(NM_CLIENT(client), result, &data.error);

  if (!data.error) {
    cout << "Added connection '"
         << nm_connection_get_path(NM_CONNECTION(remote)) << "'" << endl;
...
/// @param privkey The path to the private key file (certificate should be in
/// the same path with .crt extension)
/// @param password The password for the private key
static void netsetup(const char *privkey = "private.key",
                     const char *password = "password") {
...

  {
    ...
    g_object_set(G_OBJECT(s_con), NM_SETTING_CONNECTION_UUID, uuid,
                 ...
                 NM_SETTING_CONNECTION_TYPE, "802-3-ethernet", NULL);
    g_free(uuid);
...

  {
    ...
    g_object_set(G_OBJECT(s_8021x), NM_SETTING_802_1X_OPTIONAL, TRUE, NULL);
...

    {
      ...
      nm_setting_802_1x_set_private_key(
          s_8021x, get_filename(privkey).c_str(), password,
      ...
      NM_SETTING_802_1X_CK_FORMAT_UNKNOWN; nm_setting_802_1x_set_ca_cert (
                      s_8021x,
              ...
              if(error) {
                      Logger::String{(const char *)  error->message,"
      (NM_SETTING_802_1X_CK_SCHEME_PATH)"}.error("nmsetup"); Udjat::Exception
      exception{"Erro ao definir EST CAs ",error->message}; g_error_free(error);
                      throw exception;
              }
      }
      */
...

    {
      ...
      nm_setting_802_1x_set_client_cert(
          s_8021x, get_filename(privkey, "crt").c_str(),
          NM_SETTING_802_1X_CK_SCHEME_PATH, &format, &error);
    ...
    g_object_set(G_OBJECT(s_8021x), NM_SETTING_802_1X_SYSTEM_CA_CERTS, TRUE,
                 NULL);
    ...
    */
    ...
    /*
    for(const auto &eap :
    Config::Value<std::vector<std::string>>{"network-setup","eap","tls"}) {
            if(!nm_setting_802_1x_add_eap_method(s_8021x,eap.c_str())) {
                    throw Udjat::Exception{"Erro ao adicionar método EAP
    '",eap.c_str(),"'"};
            }
    }
    */
...
int main(int argc, char *argv[]) {
  ...
  if (argc < 2) {
    ...
    printf("  netsetup     Setup NetworkManager using the generated key and "
           "certificate\n\n");
    return -1;
  }

  auto defprov =
      make_handle(OSSL_PROVIDER_load(NULL, "default"), OSSL_PROVIDER_unload);
  if (!defprov) {
    throw runtime_error("Error loading OSSL default provider");
  }

  auto tpm2prov =
      make_handle(OSSL_PROVIDER_load(NULL, "tpm2"), OSSL_PROVIDER_unload);
  if (!tpm2prov) {
    throw runtime_error("Error loading OSSL tpm2 provider");
  }

  // string keyfile = string{getenv("HOSTNAME")}.append(".key");
  string keyfile = "tpmtest.key";
